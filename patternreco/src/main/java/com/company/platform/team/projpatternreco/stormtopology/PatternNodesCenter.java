 ...
 */
// TODO:thread safe
public final class PatternNodesCenter {
    //Map<project-level, Map<project-level-nodeid, PatternNode>>
    private ConcurrentHashMap<PatternNodeKey, PatternNode> patternNodes;
    private static PatternNodesCenter forest;
    private static final Gson gson = new Gson();
    private ReadWriteLock lock;


    public static synchronized PatternNodesCenter getInstance() {
        if (forest == null) {
            forest = new PatternNodesCenter();
        }
        return forest;
    }

    private PatternNodesCenter() {
    ...
    public Pair<PatternNodeKey, List<String>> mergePatternToNode(PatternNodeKey key,
            ...
            parentNode.setLastupdatedTime(System.currentTimeMillis());
            if ( updateNode(key, parentNode) == 0) {
                return null;
            }
